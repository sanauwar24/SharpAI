...
const GenerateImages = () => {
  const imageStyle = ['Realistic', 'Ghibli Art', 'Cartoon style',
    'Fantasy Style', 'Hyper Realistic', '3D Style', 'Prtrait', 'Food']
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <p className='mt-6 text-sm font-medium'>Describe your Style</p>
        ...
        <textarea onChange={(e) => setInput(e.target.value)} value={input} rows={4}
          ...
          border border-gray-300' placeholder='Describe what you want to see in image...' required />
          ...
          {imageStyle.map((item) => (
            <span onClick={() => setSelectedStyle(item)}
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer
               ${selectedStyle === item ? 'bg-emerald-50 text-emerald-800' : 'text-gray-500 border-gray-300'}`}
              key={item}>{item}</span>
          ))}
        </div>
        ...
        <button className='w-full flex justify-center items-center gap-2
         bg-gradient-to-r from-green-500 via-teal-700 to-emerald-800 text-white 
