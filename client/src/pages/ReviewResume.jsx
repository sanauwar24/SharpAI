    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input onChange={(e) => setInput(e.target.files[0])} accept='application/pdf' type='file'
          className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md text-gray-600
          border border-gray-300'  required />

        <p className='mt-2 text-sm font-light text-gray-500'> Supports PDF File Only </p>

        <button className='w-full flex justify-center items-center gap-2
         bg-gradient-to-r from-cyan-500 via-cyan-700 to-cyan-900 text-white 
      ...
      flex flex-col border border-gray-200 min-h-96 '>
        ...
        <div className='flex items-center gap-3'>
          ...
          <h1 className='text-xl font-semibold'>Analyzed Resume</h1>
        </div>

        <div className='flex-1 flex justify-center items-center'>
          <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
            <FileText className='w-9 h-9' />
            <p>Upload a PDF file and click "Analysis Resume" to get started</p>
          </div>
        </div>
